...
if __name__ == '__main__':
    ...
    opt.dataset_mode = "facetex"
    paired_dataset = create_dataset(opt)  # create a dataset given opt.dataset_mode and other options
    
    dataset_size = len(paired_dataset)    # get the number of images in the dataset.
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(paired_dataset):  # inner loop within one epoch
